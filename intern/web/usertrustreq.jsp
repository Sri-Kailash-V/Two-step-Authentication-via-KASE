...
<html>
    <head>
        <title>User Trustee Request</title>
    ...
    <div style="margin-left: 10%;padding: 3% 4%;background-color: rgba(225,225,225,.5);border-radius: 70px">
        ...
        <table class="tts">
            ...
            <tr>
                ...
                <% if (rs.getString("status").equalsIgnoreCase("requested")) { %>
                <td><% out.println(rs.getString("status"));%></td>
                <% } else {%>
                <td> <a style = "color: red;" href = "usertrustres.jsp?rfn=<%= rs.getString("filename")%>&upon=<%= rs.getString("uploadedon")%>&size=<%= rs.getString("size")%>&st=<%= rs.getString("status")%>">
                        <% out.println(rs.getString("status"));%></a></td>
                        <% } %>
            </tr>
            <%     }
                PreparedStatement pst = c.prepareStatement("select * from aufiles where filename not in (select filename from userfiles where emailid=?);");
                pst.setString(1, rby);
                rs = pst.executeQuery();
                while (rs.next()) {
            %>
            <tr>
                ...
                <td><% out.println(rs.getInt("size"));%></td>
                <td><a style="color: red;" href="usertrustreqto.jsp?rfn=<%= rs.getString("filename")%>&upon=<%= rs.getString("date")%>&size=<%= rs.getString("size")%>&st=Request">
                        Request</a></td>
            </tr>
            <% }
                } catch (Exception e) {
